		...
		for (int j = 0; j < col; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					sum1 += SobelX.at<float>(a + n, b + n) * (float)(input.at<G>(tempa, tempb));
					sum2 += SobelY.at<float>(a + n, b + n) * (float)(input.at<G>(tempa, tempb));


				}
			}
			output.at<G>(i, j) = (G)sqrt(sum1*sum1 +sum2*sum2);
		}
	}
