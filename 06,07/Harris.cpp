		...
		for (int j = radius; j < col + radius; j++) {
			...
			for (int x = -radius; x <= radius; x++) {
				for (int y = -radius; y <= radius; y++) {
					if (max < input_mirror.at<float>(i + x, j + y)) {
						max = input_mirror.at<float>(i + x, j + y);
						//printf("%f\n", max);
					}
				}
			}
			...
			for(int x = -radius; x <= radius; x++) {
				for (int y = -radius; y <= radius; y++) {
					if ((i + x - radius >= 0) && (i + x - radius) < row && (j + y - radius) >= 0 && (j + y - radius) < col) {
						if (max > input_mirror.at<float>(i + x, j + y)) {
							corner_mat.at<uchar>(i + x - radius, j + y - radius) = 0;
						}
					}
