...
int main() {
	...
	Mat randIdx(1, K, CV_32S);	// random index array
	int max_num = 0;
	vector<Point2f> inlierPts_1; vector<Point2f> inlierPts_2;

	srand((int)time(NULL));

	for (int i = 0; i < S; i++) {
		for (int j = 0; j < K; j++)
			randIdx.at<int>(0, j) = rand() % dstPoints.size();

		vector<Point2f> inliers_1; vector<Point2f> inliers_2;

		vector<Point2f> randPoints_src;
		vector<Point2f> randPoints_dst;

		for (int j = 0; j < K; j++) {
			randPoints_src.push_back(srcPoints[randIdx.at<int>(0, j)]);
			randPoints_dst.push_back(dstPoints[randIdx.at<int>(0, j)]);
		}
		Mat A12_r = cal_affine(randPoints_dst, randPoints_src, K);
		int num_inliers = 0;

		for (int j = 0; j < dstPoints.size(); j++) {
			Mat p(2, 6, CV_32F, Scalar(0));
			p.at<float>(0, 0) = p.at<float>(1, 3) = dstPoints[j].y;
			p.at<float>(0, 1) = p.at<float>(1, 4) = dstPoints[j].x;
			p.at<float>(0, 2) = p.at<float>(1, 5) = 1.0;
			Mat temp = p * A12_r;
			double norm_L2 = norm(temp, Mat(srcPoints[j]), NORM_L2);
			if (pow(norm_L2, 2) < DELTA * DELTA) {
				num_inliers++;
				inliers_1.push_back(srcPoints[j]);
				inliers_2.push_back(dstPoints[j]);
			}
		}
		if (max_num < num_inliers) {
			max_num = num_inliers;
			inlierPts_1 = inliers_1;
			inlierPts_2 = inliers_2;
		}

	}


	Mat A12 = cal_affine(inlierPts_2, inlierPts_1, inlierPts_2.size());
	Mat A21 = cal_affine(inlierPts_1, inlierPts_2, inlierPts_1.size());
