...
def val_model(model, valloader, criterion, device, dir):
    ...
    for batch, (inputs, labels) in enumerate(valloader):
        with torch.no_grad():
            ...
            outputs = np.transpose(outputs.cpu(), (0, 2, 3, 1)) # (N, H, W, C)
            preds = torch.argmax(outputs, dim=3).float() #(N, H, W)
            ...
            for i in range(preds.shape[0]):
                temp = preds[i].cpu().data.numpy() # (H, W)
                temp_l = labels[i].cpu().data.numpy()
                temp_rgb = np.zeros((temp.shape[0], temp.shape[1], 3)) # (H, W, 3)
                temp_label = np.zeros((temp.shape[0], temp.shape[1], 3))    # (H, W, 3)
